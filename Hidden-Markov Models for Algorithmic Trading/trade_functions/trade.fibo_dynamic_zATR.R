    ...
    if (in.pos) {
      if ((side == "long"  && as.numeric(row$Low)  <= sl.px) ||
        ...
        tp.multiplier <- if(tp.bracket == "dynamic") {
          vol.score <- row$zTick.Vol
          if (vol.score < -0.5) 1.0 else if (vol.score > 0.5) 2.0 else 1.5
        } else if (is.numeric(tp.bracket)) {
          tp.bracket
        } else 1.0
        # Apply TP level
        if (side == "long" && as.numeric(row$High) >= entry.px + tp.multiplier * (as.numeric(row$BBmavg - entry.px))) {
          exit.px <- entry.px + tp.multiplier * (as.numeric(row$BBmavg - entry.px))
          reason <- "TP"
        } else if (side == "short" && as.numeric(row$Low) <= entry.px - tp.multiplier * (as.numeric(row$BBmavg - entry.px))) {
          exit.px <- entry.px - tp.multiplier * (as.numeric(row$BBmavg - entry.px))
          reason <- "TP"
        } else {
          next
...
tradesdynamicATRHMM1 <- trade.fibo(data = bt.set.1,fib.level = 0.618, sl.buffer = 100, slippage = 0.0, 
                                   comission = 3, lot.size = 1e5, pip.value = 10, 
                                   state = c(1, 2), tp.bracket = "dynamic")

sum(tradesdynamicATRHMM1$net.pips)


tradesdynamicATRHMM1$entry
tradesdynamicATRHMM1$exit
tradesdynamicATRHMM1$gross.pips
